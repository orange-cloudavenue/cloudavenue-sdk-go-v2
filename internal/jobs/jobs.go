...
type (
	Client interface {
		...
		JobStatusParser(status string) (s Status, err error)
	}
)

// NewJobMiddleware creates a new job middleware for handling job responses.
func NewJobMiddleware(httpC *resty.Client, c Client, jobOpts *JobOptions) resty.ResponseMiddleware {
...
var jobRetryCondition = func(c Client) resty.RetryConditionFunc {
